...
// function to generate a new default key (used by github actions workflow)
public class KeyGenerator {
    public static void main(String[] args) {
        ...
        int version = (args.length > 0) ? Integer.parseInt(args[0]) : 1; // version passed via args

        String json = String.format("{\"version\": %d, \"key\": \"%s\"}", version, key);
        System.out.println(json); // redirect to GitHub Actions
    }
}